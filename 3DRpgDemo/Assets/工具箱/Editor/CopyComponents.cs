...
namespace FACS01.Utilities
{
    public class CopyComponents : EditorWindow
    {
        ...
        [MenuItem("工具箱/批量复制", false, 1005)]
        public static void ShowWindow()
        {
            window = GetWindow(typeof(CopyComponents), false, "批量复制器", true);
        ...
        public void OnGUI()
        {
            ...
            EditorGUILayout.LabelField($"<color=cyan><b>批量复制</b></color>\n扫描被复制的模型中的所有组件 " +
                $"然后选取想要复制走的组件之后复制到目标模型中", FacsGUIStyles.helpbox);

            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.BeginVertical();
            EditorGUILayout.LabelField($"<b>被复制体</b>", FacsGUIStyles.helpbox);
            ...
            if (copyFrom != null)
            {
                if (GUILayout.Button("扫描!", FacsGUIStyles.button, GUILayout.Height(40))) { GetAvailableComponentTypes(); }
            }
            ...
            if (copyComponents != null)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button("全选", FacsGUIStyles.button, GUILayout.Height(20))) { SelectAll(true); }
                if (GUILayout.Button("全不选", FacsGUIStyles.button, GUILayout.Height(20))) { SelectAll(false); }
                EditorGUILayout.EndHorizontal();
        ...
        private void CopyComponentsSerializedData(Transform t_from, Transform t_to)
        {
            ...
            for (int j = 0; j < ComponentArrayTuplesCount; j++)
            {
                ...
                {
                    componentCount++;
                    EditorUtility.DisplayProgressBar("批量复制", "请稍等...", (float)componentCount / TotalComponents);
